...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_feedback'])) {
    ...
    SELECT CONCAT(s.first_name, ' ', s.last_name) AS member_name
    FROM feedback f
    INNER JOIN signup s ON f.member_id = s.id
    WHERE f.feedback_id = ?
    LIMIT 1
");

$nameStmt->bind_param("i", $feedback_id);
$nameStmt->execute();

$nameResult = $nameStmt->get_result();
$feedbackInfo = $nameResult->fetch_assoc();

$nameStmt->close();

$memberName = $feedbackInfo['member_name'] ?? "Unknown Member";
...
");
    ...
    logActivity(
        $con,
        "Feedback Deletion",
        "Feedback submitted by " . $memberName . " was deleted.",
