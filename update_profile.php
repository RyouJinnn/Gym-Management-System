...
if ($stmt->execute()) {
    ...
    $memberIdStmt = $con->prepare("SELECT id, first_name, last_name FROM signup WHERE email=? LIMIT 1");
    $memberIdStmt->bind_param("s", $email);
    $memberIdStmt->execute();
    $updatedUser = $memberIdStmt->get_result()->fetch_assoc();
    $memberIdStmt->close();

    logActivity(
        $con,
        "Profile Update",
        $updatedUser['first_name'] . " " . $updatedUser['last_name'] . " updated their profile.",
        $updatedUser['id'],
        "Member"
    );
