...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($memberId <= 0 && $payerName === '') {

    $message = "Please select a member or enter a walk-in name.";
    $messageType = "error";
    ...
    logActivity(
        $con,
        "Member Check-in",
        $memberName . " was checked in by " . ($_SESSION['admin_name'] ?? "Admin") . ".",
        $_SESSION['admin_id'] ?? null,
        "Admin"
    );
    ...
    logActivity(
        $con,
        "Member Check-out",
        $memberName . " was checked out by " . ($_SESSION['admin_name'] ?? "Admin") . ".",
        $_SESSION['admin_id'] ?? null,
        "Admin"
    );
...
<head>
    ...
    <title>Record Attendance | Admin Panel</title>
...
<div class="wrapper">
    ...
    >
        ...
        Back to Attendance
            ...
            <div class="attendance-add-container">
                ...
                <form
                    method="GET"
                    action="attendance_add_admin.php"
                ...
                <div class="member-list">
                    ...
                    <?php if ($membersResult->num_rows > 0 || count($walkIns) > 0): ?>
    ...
    <?php
        $walkInName = $walkIn['payer_name'];
        $initial = strtoupper(substr($walkInName, 0, 1));

        $walkInTodayStmt = $con->prepare("
            SELECT *
            FROM attendance
            WHERE payer_name = ?
        ...
        $walkInTodayStmt->bind_param(
            "s",
            $walkInName
        );

        $walkInTodayStmt->execute();

        $walkInAttendance = $walkInTodayStmt
            ->get_result()
            ->fetch_assoc();

        $walkInTodayStmt->close();
    ?>
    ...
    <div class="member-row">
        ...
        <div class="attendance-actions">
            ...
            <?php if (!$walkInAttendance): ?>
                ...
                <form method="POST">
                    ...
                    <input
                        type="hidden"
                        name="payer_name"
                        value="<?= htmlspecialchars($walkInName) ?>"
                    >
            ...
            <?php elseif (empty($walkInAttendance['check_out'])): ?>
