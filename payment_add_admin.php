    ...
    if($plan){
        ...
        if($existingMembership){
            ...
            logActivity(
                ...
                " was activated after an approved payment.",
...
$qrStmt = $con->prepare("
    ...
    $qrStmt->bind_param("i", $member_id);
    $qrStmt->execute();

    $qrResult = $qrStmt->get_result();
    $memberQR = $qrResult->fetch_assoc();

    $qrStmt->close();

    // Create a QR token only if the member does not have one yet
    if(!$memberQR || empty($memberQR['qr_token'])){

        $qr_token = bin2hex(random_bytes(32));

        $updateQR = $con->prepare("
            UPDATE signup
            SET qr_token = ?
            WHERE id = ?
        ");
...
logActivity(
    ...
    header(
        "Location: payments_admin.php?added=1"
    );
